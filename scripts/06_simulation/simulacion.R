...
preparar_datos <- function(frecuencia, periodo = "invierno") {
  if (frecuencia == "horario") {
    ...
    # -------------------------------------------------------------------------
    # PERFIL HORARIO SEMANAL (laborable / fin de semana) por mes
    # Para los primeros 9 meses de 2025, se calcula por estacion el perfil
    # horario medio de dias laborables (L-V) y de fin de semana (S-D) en cada
    # mes. Resultado: 9 meses x 2 tipos de dia x 24 horas = 432 "perfiles-hora".
    # Objetivo: colapsar el ruido dia-a-dia conservando el ciclo diurno (senal
    # funcional) y el contraste laborable/finde. Mucho mas ligero que el horario
    # crudo, y el ciclo diurno hace que el AR1 sea detectable si es necesario.
    # -------------------------------------------------------------------------
    dt <- readRDS(here(
      "data", "processed", "Maestro", "horario",
      "dataset_maestro_inla_2025_HORARIO.rds"
    ...
    # Primeros 9 meses + tipo de dia (L-V vs S-D)
    dt[, MES := as.integer(format(FECHA, "%m"))]
    dt <- dt[MES <= 9]
    dt[, dow := as.integer(format(FECHA, "%u"))] # 1=Lun ... 7=Dom
    dt[, tipo_dia := fifelse(dow <= 5, "laborable", "finde")]

    # Media horaria por (estacion, mes, tipo de dia, hora)
    ...
    # Indice temporal ordenado: MES -> tipo de dia (lab<finde) -> HORA
    ...
    # Descartar celdas incompletas (respuesta o covariables NaN)
    dt <- dt[complete.cases(dt[, c("LOG_NO2", covs_cols), with = FALSE])]
    setorder(dt, ID_TIEMPO_SIM, ESTACION)

    n_periodos <- uniqueN(dt$ID_TIEMPO_SIM)
    # Train: meses 1-7 | Test: meses 8-9 (holdout temporal, ~22% de los perfiles)
    periodos_train <- sort(unique(dt[MES <= 7, ID_TIEMPO_SIM]))
    periodos_test <- sort(unique(dt[MES >= 8, ID_TIEMPO_SIM]))
    n_train_p <- length(periodos_train)
    n_test_p <- length(periodos_test)
    dt[, es_train := ID_TIEMPO_SIM %in% periodos_train]
    lab_train <- sprintf("Meses 1-7 · lab+finde × 24h (%d perfiles-hora)", n_train_p)
    lab_test <- sprintf("Meses 8-9 · lab+finde × 24h (%d perfiles-hora)", n_test_p)
...
seleccion_por_residuos_parciales <- function(datos, covs_nombres, covs_alias,
  ...
  spde_rp <- inla.spde2.matern(mesh = malla_rp, alpha = 2)
  ...
  for (paso in seq_along(vars_pool)) {
    ...
    fml_mejor <- y ~ 0 + intercept + cov_test +
      f(campo_espacial, model = spde_rp)
...
seleccionar_covariables <- function(datos, carpeta_sel,
  ...
  ajustar_sw <- function(vars_mod) {
    fml_sw <- as.formula(paste(
      "y ~ 0 + intercept +", paste(vars_mod, collapse = " + "),
      "+ f(campo_espacial, model = spde_ref)"
    ))
    stk_sw <- inla.stack(
...
guardar_diagnosticos_modelo <- function(modelo, spde_obj, carpeta_diag,
  ...
  spde_res <- inla.spde2.result(
    inla = modelo, name = "campo_espacial",
    spde = spde_obj
  )

  rango_mean <- inla.emarginal(
    function(x) x,
    spde_res$marginals.range.nominal[[1]]
  )
  var_mean <- inla.emarginal(
    function(x) x,
    spde_res$marginals.variance.nominal[[1]]
  )

  # IC 95% via cuantiles de la log-posterior transformada
  rango_q <- exp(spde_res$summary.log.range.nominal[1, c("0.025quant", "0.975quant")])
  var_q <- exp(spde_res$summary.log.variance.nominal[1, c("0.025quant", "0.975quant")])
  ...
  guardar_tabla_png(
    ...
    subtitulo = "Rango: distancia a la que la correlacion decae a ~0.13 | Varianza: intensidad del campo",
    ruta_png = file.path(
      carpeta_diag,
...
calcular_recuperacion <- function(modelo_ar1, spde_obj, datos, sel,
  ...
  cat(sprintf(
    "  [B7] rho = %.3f [%.3f, %.3f] | tau = %.2f periodos [%.2f, %.2f]\n",
...
ejecuciones <- list(
  # --- Estudio completo: descomenta para incluir mensual y diario ---
  # list(freq = "mensual", periodo = "multianual"),
  # list(freq = "diario",  periodo = "invierno"),
  # list(freq = "diario",  periodo = "verano"),
  # --- Horario crudo: 5 dias (4 train / 1 test) ---
  list(freq = "horario", periodo = "invierno"),
  # --- Horario agregado: perfil semanal laborable/finde por mes (ene-sep) ---
  list(freq = "horario_perfil", periodo = "2025_9meses")
)
...
for (ejec in ejecuciones) {
  ...
  carpeta <- here("outputs", "analysis", "simulacion", freq, periodo)
  ...
  cat(sprintf(
    "  %d filas | %d estaciones | %d periodos (train=%d, test=%d)\n",
  ...
  if (!is.null(res_parciales$tabla)) {
    ...
    tbl_parc[, Frecuencia := toupper(freq)]
...
guardar_tabla_png(
  ...
  ruta_png = here("outputs", "analysis", "simulacion", "tabla_resumen_global.png"),
  ancho_px = 1100
)
...
guardar_tabla_png(
  ...
  ruta_png = here("outputs", "analysis", "simulacion", "tabla_resultados_completos.png"),
  ancho_px = 1300
)
...
if (length(resumen_parciales) > 0) {
  ...
  guardar_tabla_png(
    ...
    ruta_png = here("outputs", "analysis", "simulacion", "tabla_macro_micro_covariables.png"),
    ancho_px = 1000
  )
...
cat(strrep("=", 80), "\n")
cat("  Resultados guardados en:", here("outputs", "analysis", "simulacion"), "\n")
