...
# Outputs: outputs/similitud_estaciones_clima/
# ==============================================================================
...
dt_d <- readRDS(here("data", "processed", "Clima", "diario",
                     "meteo_madrid_2025_diario.rds"))
setDT(dt_d)

dt_h <- readRDS(here("data", "processed", "Clima", "horario",
...
correlacion_pares <- function(dt_largo, clave_tiempo) {
  ...
  m <- as.matrix(ancho[, -1])
  if (ncol(m) < 2) {
    return(list(media = NA_real_, minima = NA_real_, n_est = ncol(m),
                r_por_estacion = NULL))
  }
...
guardar_tabla_png_local <- function(df, titulo, subtitulo, ruta_png) {
  ...
  ggsave(ruta_png, combinado, width = 10,
...
for (vi in vars_info) {
  ...
  cor_h <- correlacion_pares(d_horario[, .(FECHA, DATETIME, ESTACION, valor)],
                             "DATETIME")
  ...
  p_map_val <- ggplot() +
    geom_sf(data = distritos, fill = "grey97", colour = "grey80",
            linewidth = 0.3) +
    geom_sf(data = sf_mapa, aes(color = media_anual), size = 4.2,
            alpha = 0.95) +
    geom_sf_text(data = sf_mapa,
  ...
  fig_mapa <- gridExtra::arrangeGrob(
    ...
    grid::textGrob(
      paste("(a) niveles medios anuales · (b) correlación diaria de cada",
  ...
  archivo_mapa <- file.path(dir_salida,
                            sprintf("mapa_similitud_%s.png", vi$file))
  ggsave(archivo_mapa, fig_mapa, width = 13, height = 7, dpi = 200,
         bg = "white")
  ...
  for (nom_temp in names(temporadas)) {
    ...
    paneles_dia[[nom_temp]] <- ggplot() +
      ...
      geom_sf_text(data = sf_dia, aes(label = sprintf("%.1f", valor)),
                   size = 2.6, nudge_y = 1300, color = "grey15") +
      scale_color_viridis_c(name = NULL) +
      labs(
        title = sprintf("%s · %s", etiquetas_temporada[[nom_temp]],
                        format(dia, "%d %b %Y")),
        subtitle = sprintf("rango: %.1f – %.1f  |  sd entre estaciones = %.2f",
  ...
  if (length(paneles_dia) > 0) {
    fig_dias <- gridExtra::arrangeGrob(
      grid::textGrob(
        sprintf("%s — Valores diarios por estación en un día de cada temporada · Madrid 2025",
                vi$ylab),
        gp = grid::gpar(fontsize = 13, fontface = "bold")
      ),
      grid::textGrob(
        paste("Lectura directa de la correlación espacial: colores/valores",
    ...
    ggsave(archivo_dias, fig_dias, width = 12, height = 12.5, dpi = 200,
           bg = "white")
