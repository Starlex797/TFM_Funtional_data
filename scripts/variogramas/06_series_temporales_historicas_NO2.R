...
cargar_anios <- function(escala, patron_archivo, patron_cols) {
  rbindlist(
    lapply(ANIOS, function(a) {
      ruta <- here("data", "processed", "contaminacion", escala,
...
cat(sprintf("  Diario  : %d obs · %d estaciones\n",
            nrow(dt_diario),  uniqueN(dt_diario$ESTACION)))
cat(sprintf("  Mensual : %d obs · %d estaciones\n",
            nrow(dt_mensual), uniqueN(dt_mensual$ESTACION)))
cat(sprintf("  Horario : %d obs · %d estaciones\n",
...
for (est in estaciones_unicas) {
  ...
  if (nrow(dt_est) < 2L) { cat(sprintf("  [SKIP diario] %s\n", est)); next }
  ...
  p <- ggplot(dt_est, aes(x = FECHA, y = DATO_DIARIO)) +
    ...
    annotate("text", x = as.Date("2019-02-01"), y = 42,
    ...
    scale_x_date(
      ...
      labels       = function(d) ifelse(format(d, "%m") == "01",
                                        format(d, "%b\n%Y"),
                                        format(d, "%b")),
      minor_breaks = NULL,
...
for (est in estaciones_unicas) {
  ...
  if (nrow(dt_est) < 2L) { cat(sprintf("  [SKIP mensual] %s\n", est)); next }
  ...
  nombre_file <- file.path(dir_mensual,
...
for (est in estaciones_unicas) {
  ...
  if (nrow(dt_est) < 2L) { cat(sprintf("  [SKIP horario] %s\n", est)); next }
...
# ── Panel mensual ─────────────────────────────────────────────────────────────
p_panel_m <- ggplot(dt_mensual[!is.na(DATO_MENSUAL)],
  ...
  geom_point(color = "#2980b9", size = 0.9, shape = 21,
  ...
  scale_x_date(breaks = primeros_anios, labels = etiquetas_anios,
               minor_breaks = NULL, expand = expansion(mult = c(0.005, 0.005))) +
  facet_wrap(~ ESTACION, ncol = 4, scales = "free_y") +
  labs(title    = "Series Mensuales de NO\u2082 \u2014 Todas las Estaciones",
       subtitle = "Madrid 2019\u20132025  \u00b7  L\u00ednea roja: l\u00edmite OMS (40 \u00b5g/m\u00b3)",
       x = NULL, y = "NO\u2082 (\u00b5g/m\u00b3)") +
  tema_panel()
...
# ── Panel horario ─────────────────────────────────────────────────────────────
p_panel_h <- ggplot(dt_horario[!is.na(DATO)],
                    aes(x = DATETIME, y = DATO)) +
  geom_rect(data = franjas_anios_ct,
            aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf),
            inherit.aes = FALSE, fill = "gray88", alpha = 0.5) +
  geom_vline(xintercept = primeros_anios_ct,
             color = "gray45", linewidth = 0.35) +
  geom_line(color = "#2c3e50", linewidth = 0.2, alpha = 0.6) +
  geom_hline(yintercept = 40, color = "#e74c3c",
             linewidth = 0.35, linetype = "longdash", alpha = 0.7) +
  scale_x_datetime(breaks = primeros_anios_ct, labels = etiquetas_anios,
                   minor_breaks = NULL, expand = expansion(mult = c(0.005, 0.005))) +
  facet_wrap(~ ESTACION, ncol = 4, scales = "free_y") +
  labs(title    = "Series Horarias de NO\u2082 \u2014 Todas las Estaciones",
       subtitle = "Madrid 2019\u20132025  \u00b7  L\u00ednea roja: l\u00edmite OMS (40 \u00b5g/m\u00b3)",
       x = NULL, y = "NO\u2082 (\u00b5g/m\u00b3)") +
  tema_panel()
