...
p_comp <- ggplot(perfil_comp,
  ...
  labs(
    ...
    subtitle = paste0("Todas las estaciones de medici\u00f3n  \u00b7  ",
...
ggsave(archivo_comp, plot = p_comp,
...
cat(sprintf("  Registros válidos (laborables)   : %d\n",
            dt_h[tipo_dia == "Laborable" & !is.na(DATO), .N]))
cat(sprintf("  Registros válidos (finde)        : %d\n",
