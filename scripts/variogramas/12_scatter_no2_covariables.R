...
vars_cov <- list(
  list(col_d = "Temperatura_raw",   col_h = "Temperatura_raw",
       label  = "Temperatura (\u00b0C)",
       file   = "temperatura"),

  list(col_d = "Humedad_Relativa_raw", col_h = "Humedad_Relativa_raw",
       label  = "Humedad relativa (%)",
       file   = "humedad"),

  list(col_d = "Precipitaciones_raw", col_h = "Precipitaciones_raw",
       label  = "Precipitaciones (mm)",
       file   = "precipitacion"),

  list(col_d = col_presion_d,   col_h = col_presion_h,
       label  = "Presi\u00f3n barom\u00e9trica (mbar)",
       file   = "presion"),

  list(col_d = col_radiacion_d, col_h = col_radiacion_h,
       label  = "Radiaci\u00f3n solar (W/m\u00b2)",
       file   = "radiacion"),

  list(col_d = col_viento_d,   col_h = col_viento_h,
       label  = "Velocidad del viento (m/s)",
       file   = "viento"),

  list(col_d = "intensidad_raw", col_h = "intensidad_raw",
       label  = "Intensidad tr\u00e1fico (veh/h)",
       file   = "intensidad"),

  list(col_d = "carga_raw",      col_h = "carga_raw",
       label  = "Carga tr\u00e1fico",
       file   = "carga")
)
...
hacer_scatter <- function(datos, col_x, col_y, label_x, label_y,
  ...
  dt_p <- datos[!is.na(get(col_x)) & !is.na(get(col_y)),
  ...
  ggplot(dt_p, aes(x = X, y = Y, color = ESTACION)) +
    ...
    labs(
      title    = titulo,
      subtitle = sprintf("%s  \u00b7  %s observaciones v\u00e1lidas",
